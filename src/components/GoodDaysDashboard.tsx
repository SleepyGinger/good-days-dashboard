...
export default function GoodDaysDashboard() {
  ...
  useEffect(() => {
    if (byDate.length) setRand(byDate[Math.floor(Math.random() * byDate.length)]);
  }, [byDate]);
  ...
  const [month, setMonth] = useState(() => new Date(daysWithNotes[0] ?? new Date()));
    ...
    <div className="dark bg-gray-900 min-h-screen text-gray-100 p-4 space-y-6">
      ...
      <header className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            variant="ghost"
            onClick={() => { setEditingTheme(null); setThemeDrawerOpen(true); }}
          ...
          {byDate.length > 0 && (
            <Button
              variant="ghost"
              onClick={() => setRand(byDate[Math.floor(Math.random() * byDate.length)])}
              className="flex gap-2"
            >
          ...
          {ribbons.map((r) => (
            <div
              ...
              onClick={() => {
                ...
                if (t) { setEditingTheme(t); setThemeDrawerOpen(true); }
              }}
              className="absolute"
              style={{
              ...
              {drawerData.items.map((item, i) => (
                <Card key={i} className="bg-gray-800">
                  <CardContent className="p-4 space-y-2 text-sm">
                    <p>Day: <strong>{item.day}</strong></p>
                    <p>Energy: <strong>{item.energy}</strong></p>
                    <p>Touch: <strong>{item.touch}</strong></p>
                    {item.note && (
      ...
      <ThemeDrawer
        ...
        onOpenChange={(b) => { setThemeDrawerOpen(b); if (!b) setEditingTheme(null); }}
